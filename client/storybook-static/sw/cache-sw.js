		...
		caches
			...
			.then(cache =>
				fetch('/sw/asset-manifest.json')
					...
					.then(assets => {
						let urlsToCache = ['/'];
						if (!!assets && !!Object.values(assets).length)
							urlsToCache = urlsToCache.concat(Object.values(assets));
...
self.addEventListener('fetch', event => {
	...
	event.respondWith(
		networkOrCache(event.request).catch(error => (event.request.method === 'GET' ? useFallback() : error)),
	);
});

function networkOrCache(request) {
	return (
		fromNetwork(request.clone(), TIMEOUT_FETCH_MODE ? TIMEOUT_BEFORE_CACHE_USED : false)
			.then(response => {
				return response;
...
function update(request) {
	return caches
		.open(CACHE_STORAGE_NAME)
		.then(cache =>
			fetch(request.clone()).then(response => cache.put(request.clone(), response.clone()).then(() => response)),
		);
}
...
function fromCache(request) {
	return caches
		.open(CACHE_STORAGE_NAME)
		.then(cache => cache.match(request).then(matching => matching || Promise.reject('no-cache-store-match')));
}
