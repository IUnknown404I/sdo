...
export class EmailValidationPipe implements PipeTransform {
	transform(value: any, metadata: ArgumentMetadata) {
		if (typeof value !== 'string') throw new NotValidDataError();
			...
			throw new NotValidDataError('Минимальная длина электронной почты 6 символов!');
		else if (value.trim().length > 64)
			throw new NotValidDataError('Превышена максимальная длина электронной почты!');
		else if (value.trim().match(emailRegex).length === 0)
